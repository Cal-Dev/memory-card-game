...
export class MainComponent implements OnInit {
  ...
  startNewGame() {
    ...
    for (let i = 1; i <= this.size; i++) {
      ...
      let card = new Card(i, first, currentIcon);
  ...
  switchPosition(card: Card) {
    if (card && !card.isFound && this.cardBuffer.length !== 2) {
      ...
      if (this.cardBuffer.length === 2) {
        setTimeout(() => {
          if(this.sameCards()) {
            this.cardBuffer[0].isFound = true;
            this.cardBuffer[1].isFound = true;
          }
          this.cardBuffer = new Array<Card>();
          
          for (let card of this.cards.filter(x => !x.isFound)) {
  ...
  sameCards(): boolean {
    return this.cardBuffer[0].pairId === this.cardBuffer[1].pairId;
  }


  fillIcons() {
    for (let icon of this.getAllIcons()) {
      this.addIcon(icon);
    }
  }
